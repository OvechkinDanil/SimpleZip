...
public class Manager {
    ...
    public RC execute()
    {
        ...
        try(FileInputStream fis = new FileInputStream(inputFileName))
        {
           try(FileOutputStream fos = new FileOutputStream(outputFileName)) {
               ...
               RC err = ((IReader) pipeLine[0]).execute();
    ...
    public RC makePipeLine()
    {
        ...
        pipeLine = new IPipelineStep[line.length];
        IPipelineStep worker;
    ...
    private IPipelineStep createWorker(String className)
    {
        try
        {
            ...
            return (IPipelineStep) constructor.newInstance(logger);
        }
    ...
    private RC setReaderParams()
    {
        ...
        reader.setConsumer((IConsumer) pipeLine[1]);
    ...
    private RC setWriterParams()
    {
        ...
        writer.setProducer((IProducer) pipeLine[pipeLine.length - 2]);
    ...
    private RC setExecutorsParams()
    {
        for (int i = 1; i < pipeLine.length - 1; i++)
        {
            ((IExecutor)pipeLine[i]).setConsumer((IConsumer) pipeLine[i+1]);
            ((IExecutor)pipeLine[i]).setProducer((IProducer) pipeLine[i-1]);
        }
